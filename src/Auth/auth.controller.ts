...
export class AuthController {
  ...
  @ApiOperation({
    summary: 'User login',
    description:
      'Authenticate user with email and password. Returns JWT token, user information, and sidebar modules based on role permissions.',
  })
  @ApiBody({
    type: LoginDto,
  ...
  @ApiResponse({
    status: 200,
    description:
      'Login successful - Returns JWT token, user data, and sidebar modules',
  ...
  @ApiOperation({
    ...
    description:
      'Send password reset link to user email address. The reset token will be sent via email.',
  })
  @ApiBody({
  ...
  @ApiOperation({
    ...
    description:
      'Reset user password using the token received via email. The new password must meet security requirements (min 8 characters with uppercase, lowercase, number, and special character).',
  })
  @ApiBody({
  ...
  @ApiOperation({
    summary: 'User logout',
    description:
      'Logout user and blacklist the JWT token. The token will be invalidated and cannot be used for subsequent requests.',
  })
  @ApiResponse({
    status: 200,
  ...
  async signup(@Body() signupDto: CreateStudentDto) {
    return await this.usersService.createStudent(signupDto);
  }
}
